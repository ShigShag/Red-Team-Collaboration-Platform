-- Seed system methodology templates
-- These can also be seeded via: npx tsx src/db/seed-methodology-templates.ts

INSERT INTO "methodology_templates" ("name", "category", "content", "is_system")
VALUES
  ('Web Application Penetration Test', 'web', '**Phase 1 — Reconnaissance & Information Gathering:** Passive and active reconnaissance of the target application including technology stack fingerprinting, directory and file enumeration, subdomain discovery, and analysis of publicly available information (OSINT). Identification of application entry points, authentication mechanisms, and third-party integrations.
...
**Phase 7 — Reporting & Remediation Guidance:** Documentation of all findings with reproduction steps, evidence (screenshots, HTTP requests/responses), risk ratings aligned with CVSS v3.1, and prioritized remediation recommendations including both short-term mitigations and long-term architectural improvements.', true),

  ('Active Directory / Internal Network', 'active_directory', '**Phase 1 — Network Reconnaissance & Discovery:** Host discovery, port scanning, and service enumeration across the authorized network ranges. Identification of domain controllers, DNS servers, DHCP servers, file shares, and other critical infrastructure components. Passive network traffic analysis where authorized.
...
**Phase 7 — Reporting & Remediation Guidance:** Comprehensive documentation of the attack path from initial access to domain compromise, with detailed evidence, risk ratings, and prioritized remediation recommendations addressing both immediate fixes and strategic Active Directory hardening measures.', true),

  ('External Network / Infrastructure', 'network', '**Phase 1 — Discovery & Reconnaissance:** Identification of externally-facing assets through DNS enumeration, certificate transparency log analysis, search engine discovery, and targeted port scanning. Mapping of the external attack surface including IP ranges, web servers, mail servers, VPN gateways, and remote access services.
...
**Phase 6 — Reporting & Remediation Guidance:** Detailed documentation of all findings including network diagrams, reproduction steps, CVSS v3.1 ratings, and remediation guidance prioritized by risk and effort. Strategic recommendations for improving external perimeter security posture.', true),

  ('API Security Assessment', 'api', '**Phase 1 — API Discovery & Documentation Review:** Review of API documentation (OpenAPI/Swagger, GraphQL schemas, WSDL) and identification of undocumented endpoints through traffic analysis, path brute-forcing, and source code examination where available. Mapping of API versioning, rate limiting, and available HTTP methods.
...
**Phase 6 — Reporting & Remediation Guidance:** Documentation of all findings aligned with the OWASP API Security Top 10, including reproduction steps with sample API requests/responses, CVSS v3.1 ratings, and remediation guidance covering both API-specific and application-level improvements.', true),

  ('Cloud Security Review', 'cloud', '**Phase 1 — IAM & Access Control Review:** Assessment of identity and access management configuration including user roles, service accounts, privilege escalation paths, and policy misconfigurations. Review of MFA enforcement, credential rotation policies, and federated identity configurations.
...
**Phase 6 — Reporting & Remediation Guidance:** Documentation of all findings with cloud-provider-specific remediation steps, risk ratings, and architectural recommendations for improving cloud security posture aligned with CIS Benchmarks and cloud provider security best practices.', true),

  ('Mobile Application Security', 'mobile', '**Phase 1 — Static Analysis & Reverse Engineering:** Decompilation and static analysis of the application binary to identify hardcoded credentials, API keys, insecure cryptographic implementations, and embedded secrets. Review of application permissions, code obfuscation effectiveness, and third-party library vulnerabilities.
...
**Phase 6 — Reporting & Remediation Guidance:** Documentation of all findings aligned with the OWASP Mobile Application Security Verification Standard (MASVS), including reproduction steps, CVSS v3.1 ratings, and remediation guidance covering both application-level and backend API security improvements.', true),

  ('Wireless Network Assessment', 'wireless', '**Phase 1 — Wireless Reconnaissance:** Discovery and enumeration of wireless networks within the assessment scope including SSID identification, signal strength mapping, encryption protocol identification (WPA2/WPA3/WEP), and channel analysis. Identification of hidden networks and rogue access points.
...
**Phase 5 — Reporting & Remediation Guidance:** Documentation of all findings including wireless coverage maps, reproduction steps, risk ratings, and remediation guidance covering wireless infrastructure hardening, client configuration recommendations, and monitoring improvements.', true),

  ('Red Team Engagement', 'general', '**Phase 1 — OSINT & Target Reconnaissance:** Comprehensive open-source intelligence gathering including employee enumeration, technology stack identification, social media analysis, credential breach database searches, and physical location reconnaissance. Development of target profiles and initial attack planning.
