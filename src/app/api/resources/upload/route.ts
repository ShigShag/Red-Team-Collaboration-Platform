...
export async function POST(request: NextRequest) {
  ...
  return new Promise<NextResponse>((resolve) => {
    ...
    bb.on("close", async () => {
      ...
      const { engagementId, resourceId, filename } = fields;
      if (!engagementId || !resourceId) {
        if (fileResult) {
          unlink(join(RESOURCES_DIR, fileResult.diskFilename)).catch(() => {});
        }
        resolve(
          NextResponse.json(
            { error: "Missing engagementId or resourceId" },
            { status: 400 }
          )
        );
        return;
      ...
      // Verify resource exists and belongs to the engagement
      const [resource] = await db
        .select({ id: resources.id, categoryId: resources.categoryId })
        .from(resources)
        .innerJoin(
          engagementCategories,
      ...
      const existingFiles = await db
        .select({ sortOrder: resourceFiles.sortOrder })
        .from(resourceFiles)
      ...
      const [inserted] = await db
        ...
        .returning({ id: resourceFiles.id });

      // Revalidate pages
      revalidatePath(`/engagements/${engagementId}`);
      revalidatePath(
        `/engagements/${engagementId}/categories/${resource.categoryId}`
      );

      resolve(
        NextResponse.json({ fileId: inserted.id, success: true })
      );
    });
